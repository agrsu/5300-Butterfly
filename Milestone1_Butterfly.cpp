...
string printSelectStat(const SelectStatement* stmt) {
	...
	//print "FROM" clause
	output.append(" FROM ");
	output.append(printTableInformation(stmt->fromTable));
	output.append(" ");
	...
	if (stmt->unionSelect != NULL) {
		output.append("\n\tUNION\n");
		output.append("\t " + printSelectStat(stmt->unionSelect));
		output.append(" ");
	}
...
std::string printCreateStatement(const CreateStatement* stmt2) {
	...
	for (iter = cols->begin(); iter != cols->end(); iter++) {
		...
		case 1: output += "int, "; break;
		case 2: output += "double, "; break;
		case 3: output += "text, "; break;
		}
	}
	//cleanup last occurence of ','
	string::size_type pos = output.find_last_of(",");
...
void printStatement(const SQLStatement* stmt, StatementType stmtType) {
	...
	switch (stmtType) {
	case kStmtSelect: {
		cout << printSelectStat((const SelectStatement*)stmt) << endl;
		break;
	}
	case kStmtCreate: {
